...
The **AI Proxy** plugin is not restricted to LLMs. It supports a variety of GenAI models and APIs including:

* Embeddings: Offers unified text-to-vector embedding with support for multiple providers and analytics.
* Assistants and responses: Powers persistent tool-using agents and exposes metadata for debugging and evaluation.
* Batch and files: Supports parallel LLM requests and file upload for long documents and structured input.
* Audio: Enables Speech-To-Text (STT), Text-To-Speech (TTS), and real-time translation for voice agents and multilingual UIs.
* Image generation and editing: Generates or modifies images from text prompts for multimodal agent input/output.
* Video generation: Generates videos from text prompts for multimodal agent output.
* AWS Bedrock agent APIs: Enables advanced orchestration and real-time RAG via Converse and RetrieveAndGenerate endpoints. This capability is available only while using native LLM format for Bedrock.
* HuggingFace text generation: Provides text generation and streaming using open-source Hugging Face models. This capability is available only while using native LLM format for HuggingFace.
* Rerank: Improves relevance in RAG pipelines by reordering documents based on context using Bedrock or Cohere /rerank APIs. This capability is available only while using native LLM format for Bedrock and Cohere.

![genai_apis](/static/images/genai_apis.jpg)

##  Providers
Also, the **AI Proxy plugin** supports an extensive list of GenAI providers. For example, for Core Text Generation (LLMs) the plugins supports:
